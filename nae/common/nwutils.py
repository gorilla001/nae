...
def set_fixed_ip(uuid, addr):
    ...
    try:
        ...
        subprocess.check_call("sudo ip link add %s type veth peer name %s" %
                              (veth_int, veth_ext),
                              shell=True)
        ...
        subprocess.check_call("sudo nsenter -t %s -n ip addr add %s dev eth1" %
                              (pid.strip(), IP_ADDR),
                              shell=True)
        """Set default gateway to br0's gateway"""
        DEFAULT_GATEWAY = get_default_gateway()
        LOG.info("Set default gateway to %s" % DEFAULT_GATEWAY)
        subprocess.check_call("sudo nsenter -t %s -n ip route del default" %
                              pid.strip(),
                              shell=True)
        subprocess.check_call(
            "sudo nsenter -t %s -n ip route add default via %s dev eth1" %
            (pid.strip(), DEFAULT_GATEWAY),
            shell=True)
        ...
        """Ping gateway"""
        LOG.info("Flush gateway's arp caching")
        subprocess.check_call("sudo nsenter -t %s -n ping -c 3 %s" %
                              (pid.strip(), DEFAULT_GATEWAY),
                              shell=True)
