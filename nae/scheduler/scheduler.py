...
class SimpleScheduler(driver.Scheduler):
    ...
    def run_instance(self, project_id, user_id, image_id, repos, branch, env,
                     user_key, zone_id):
        """
	...
	"""
        self.save_to_db(db_id, name, env, project_id, repos, branch, image_id,
                        user_id, host_id)
        try:
            self.post(host,
                      port,
    ...
    def _scheduler(self):
        ...
        for host in unweighted_host:
            ...
            weighted_host = WeightedHost(host.id, weight, host.host, host.port)
